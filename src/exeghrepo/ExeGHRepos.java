...
public class ExeGHRepos {
	...
	private String testCSV = "testResults.csv";
	...
	private TreeSet<String> studentRepos;
	...
	private void getURLs(String organization, String assignment, String tag) {
		...
		repoURLs = gt.getRepoURLs(organization, assignment, tag, studentRepos);
		String[] excludeRepos = excludeRepo.split(",");
		if (!"".equals(excludeRepo)) {
			for (int i = repoURLs.size() - 1; i >= 0; i--) {
				for (String excRepo : excludeRepos) {
					if (repoURLs.get(i).matches("^.*" + excRepo + ".*")) {
	...
	private void processArgs(String[] args) {
		...
		for (int i = 0; i < args.length; i++) {
			...
			switch (args[i]) {
				case "-o":
					org = getNextArg(args, i++);
					break;
				case "-a":
					assignment = getNextArg(args, i++);
					break;
				case "-p":
					path = getNextArg(args, i++);
					break;
				case "-l":
					runLog = getNextArg(args, i++);
					break;
				case "-t":
					tag = getNextArg(args, i++);
					break;
				case "-d":
					date = getNextArg(args, i++);
					break;
				case "-b":
					branch = getNextArg(args, i++);
					break;
				case "-x":
					excludeRepo = getNextArg(args, i++);
					break;
				case "-i":
					incremental = true;
					break;
	...
	private void processPostCommands(String command) {
		...
		for (int i = 0; i < tokens.length; i++) {
			if (tokens[i].matches("^~.*")) {
				tokens[i] = Paths.get(System.getProperty("user.home"), tokens[i].replaceAll("^~[^A-Za-z]*", ""))
						.toString();
			}
		}
	...
	private void updateCurrTestOption(String option, String value) {
		if (value == null)
			return;
		if ("EXE".equals(option)) {
			currTest.setTestMode(value);
		} else if ("USER".equals(option)) {
			currTest.setUser(true);
		} else if ("VIM".equals(option)) {
			currTest.setVim(value);
		} else if ("SRC".equals(option)) {
			currTest.setSourcePath(value);
		} else if ("LIB".equals(option)) {
			if (!"".equals(value)) {
				currTest.addLib(value);
			}
		} else if ("CMP".equals(option)) {
			currTest.setCmp(value);
		} else if ("ARGS".equals(option)) {
			currTest.setArgs(value);
	...
	private void processConfigOptions(String[] tokens) {
		...
		switch (option) {
			case "ORG":
				...
				return;
			case "ASSIGNMENT":
				assignment = values.get(0);
				return;
			case "DELETE":
				if (!values.isEmpty())
					processDelete(new ArrayList<String>(values));
				return;
			case "COPY":
				if (!values.isEmpty())
					processCopy(new ArrayList<String>(values));
				return;
			case "POSTCMD":
				if (!values.isEmpty())
					processPostCommands(values.get(0));
				return;
			case "TEST":
				processTest(values.get(0));
				return;
			case "ROSTER":
				useRoster = true;
				processRoster(values.get(0));
				return;
			default:
				updateCurrTestOption(option, values.get(0));
				return;
		}
	}

	private void processRoster(String roster) {
		File inf = new File(path +"/"+roster);
		if (inf.exists()) {
			try {
				...
				while ((line = br.readLine()) != null) {
					...
					if (studentRepos == null) 
						studentRepos = new TreeSet<String>();
					studentRepos.add(assignment+"-"+tokens[0].trim());
				}
				br.close();
			} catch (IOException e) {
				e.printStackTrace();
			...
			System.out.println("-I- File "+roster+" does not exist. Disabling roster restriction");
			useRoster = false;
		}
	}
	...
	private ArrayList<String> readTestHeader(String line) {
		...
		for (int i = 1; i < tokens.length; i++) {
			...
			if (!detailTestOrderMap.containsKey(test)) {
				detailTestOrderMap.put(test, new ArrayList<String>());
			}
			detailTestOrderMap.get(test).add(subTest);
			testOrder.add(tokens[i]);
		}
		return testOrder;
	}

	private void readTestDetails(ArrayList<String> testOrder, String line) {
		String[] results = line.split(",");
		String repo = results[0];
		if (!detailedTestResults.containsKey(repo))
			detailedTestResults.put(repo, new HashMap<String, ArrayList<String>>());
		int index = 0;
		for (int i = 0; i < testOrder.size(); i++) {
			String test = testOrder.get(i).replaceAll(":.*", "");
			String subTest = testOrder.get(i).replaceAll(".*:", "");
			if (!detailedTestResults.get(repo).containsKey(test))
				detailedTestResults.get(repo).put(test, new ArrayList<String>());
			if (detailTestOrderMap.get(test).get(index).equals(subTest))
				detailedTestResults.get(repo).get(test).add(subTest + ":" + results[i + 1]);
			index++;
			if (detailTestOrderMap.get(test).size() == index)
				index = 0;
		}
	}
	...
	private void processTestResultsCSV() {
		...
		if (testResultsFile.exists()) {
			...
			try (BufferedReader br = new BufferedReader(new FileReader(testResultsFile))) {
				...
				ArrayList<String> testOrder = readTestHeader(br.readLine());
				while ((line = br.readLine()) != null) {
					readTestDetails(testOrder, line);
				}
			} catch (Exception e) {
				e.printStackTrace();
			}
	...
	private boolean isFailingClone(String repo) {
		...
		for (ExeTest test : testList) {
			if (!repoTestResults.get(test.getTestName()).equals("PASSED"))
				return true;
		}
		return false;
	}
	...
	private String getRemoteHeadSHA(String url) {
		String head = (!"".equals(branch)) ? branch : "HEAD";
	...
	private boolean repoRequiresCloning(String url, String repo) {
		...
		skipRepoExe = false;
		if (!clone.exists()) // have to clone
			return true;
		if (noClone) // must not clone
			return false;
		if (force || !"".equals(date)) // need to separate these - date should only clone if necessary
			return true;
		// now that branching has become mainstream...
		boolean status = true;
		if (!"".equals(branch))
			status = switchToBranch();
		if (!status) // if the branch does not exist, must clone
			return true;
		String head = (!"".equals(branch)) ? branch : "HEAD";
	...
	private void copyDirectory(String srcLoc, String destLoc) throws IOException {
		Files.walk(Paths.get(srcLoc)).forEach(source -> {
	...
	private void readClassPathAndAddLibraries(ExeTest test) {
		File cp = new File(repoPath + "/.classpath");
		String line;
		if (cp.exists()) {
			try {
				...
				while ((line = br.readLine()) != null) {
					if (line.contains("kind=\"lib\"")) {
						String libPath = line.replaceAll(".*path=\"(.*)\".*", "$1");
						String[] tokens = libPath.split("/");
						if (!"library".equals(tokens[0])) {
							System.out.println("Unexpected library referenced on path: " + libPath);
						} else {
							if (!test.getLib().contains(libPath))
								System.out.println("Adding library: " + libPath + " to testLib");
							test.addLib(libPath);
						}
					}
				}
				...
				System.out.println("Exception occurred with trying to read " + cp.getName() + ": Aborting");
				e.printStackTrace();
			}
		}
	...
	private void setupTest(ExeTest test) throws Exception {
		readClassPathAndAddLibraries(test);
		gr.writeBuildGradleFile(repoPath, test.getTestMode(), test.getSourcePath(), test.getTestName(), test.getLib());
	...
	private void clearExistingTestResults(ExeTest test) {
		if (testResults.containsKey(currRepo)) {
			testResults.get(currRepo).put(test.getTestName(), "-");
		}
		if (detailedTestResults.containsKey(currRepo)) {
			if (detailedTestResults.get(currRepo).containsKey(test.getTestName())) {
				detailedTestResults.get(currRepo).get(test.getTestName()).clear();
			}
		}
	}
	...
	void recordDetailedTestResults(String testName, String subTestName, int index, String result) {
		...
		if (detailTestOrderMap.get(testName).get(index).equals(subTestName)) {
			if (index == detailedTestResults.get(currRepo).get(testName).size()) {
				detailedTestResults.get(currRepo).get(testName).add(subTestName + ":" + result);
			} else {
				detailedTestResults.get(currRepo).get(testName).set(index, subTestName + ":" + result);
			}
		} else {
...
//				results.getOutput().add(test.getTestName() + " > PASSED");
	...
	private void cleanUpRepo() {
		File del = null; // new File(repoPath + "/build.gradle");
//		try {
//			Files.move(Paths.get(repoPath + "/build.gradle"), Paths.get(repoPath + "/build.gradle" + ".save"),
...
//		}
	...
	private void executeVIM(ExeTest test) {
		String[] tokens = test.getVim().split("\\s+");
		String[] cmd = new String[tokens.length + 2];
		cmd[0] = "gvim";
		cmd[1] = "-p";
		for (int i = 0; i < tokens.length; i++)
			cmd[i + 2] = tokens[i];
		ProcessResults results = executeProcess(cmd, new File(repoPath + "/" + test.getSourcePath()), 0);
		return;
	}
	...
	private boolean switchToBranch() {
		...
		if (found) {
			...
			ProcessResults results = executeProcess(cmd, new File(repoPath), 0);
			if (results.getStatus() != 0) {
				System.out.println("-E- Unable to switch to branch " + branch + ". Skipping repo");
				return false;
			} else {
				results.printOutput();
			}
	...
	private void executeRepo(String url) {
		...
		if (!processRepo(url, currRepo)) {
			System.out.println("-E- Error occurred while trying to set up repo: " + currRepo);
			return;
		} else if (skipRepoExe) {
			System.out.println("-I- Incremental mode: No new changes for repo " + currRepo + " - Skipping");
			return;
		}
	...
	private void writeDetailedTestResults() {
		File detailFile = new File(path + "/" + testCSV);
		if (detailFile.exists()) {
			try {
				Files.move(Paths.get(detailFile.getPath()), Paths.get(detailFile.getPath() + ".save"),
						StandardCopyOption.REPLACE_EXISTING);
			} catch (IOException e) {
				e.printStackTrace();
			}
		...
		try {
			...
			TreeSet<String> repoSet;
			
			if (studentRepos != null) {
				repoSet = studentRepos;
			} else {
				repoSet = new TreeSet<String>(repoURLs);
			}
			for (String url : repoSet) {
				currRepo = url.replaceAll(".git$", "").replaceAll("^.*\\/", "");
	...
	private void writeHistoryFile() {
		...
		try {
			...
			TreeSet<String> repoSet;
			if (studentRepos == null) {
				repoSet = new TreeSet<String>(testResults.keySet());
			} else {
				repoSet = studentRepos;
			}
